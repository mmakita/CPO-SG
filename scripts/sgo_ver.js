	...
	if(id == 0) {
		...
		$.get('sgo.php',{
			...
			if(fb[0].success == true) {
				$("#addEtapaTable").hide();
				$(".addEtapaLink").show();
				$("#etapasTable").append('<tr class="c"><td class="c">'+ $("option:selected").html() +'</td><td class="c"><a href="javascript:void(0);" onclick="window.open(\'sgd.php?acao=ver&amp;docID='+ $("#procEtapa").val() +'\',\'detalheEtapa\',\'width=950,height=650,scrollbars=yes,resizable=yes\')">'+ $("#detalhe"+$("#procEtapa").val()).html() +'</a></td><td class="c"><a href="javascript:void(0)" onclick="window.open(\'sgo.php?acao=verEtapa&amp;etapaID='+ fb[0].etapaID +'\',\'detalheEtapa\',\'width=950,height=650,scrollbars=yes,resizable=yes\')">Ver detalhes</a></td></tr>');
