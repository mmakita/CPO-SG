<?php
	...
	if($_GET['show'] == "un" && isset($_GET['q'])){
		...
		$items = $bd->query("SELECT id, sub, CONCAT(nome,' (', sigla,')')AS nome FROM unidades WHERE id LIKE  '".$q."%' OR nome LIKE  '%".$q."%' OR sigla LIKE  '%".$q."%'");
		
		foreach ($items as $i) {
			$ret = $i['id'].' - '.$i['sub'].$i['nome']."|".$i['id']."\n";
			echo $ret;
		} 
		...
		$res = $bd->query("SELECT id, CONCAT( nome,' (', sigla,')') AS nome FROM  unidades WHERE id LIKE '".$_GET['id'].".__' ORDER BY nome ASC");
		print(json_encode($res));
