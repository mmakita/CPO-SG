    	...
    	//para cada obra encontrada, cria um marcador e uma entrada na tabela
	   	$.each(obras, function(i){
			...
			if(!obras[i].descr.valor)
				...
				var descr = obras[i].descr.label+'<br /><br />';
			...
			google.maps.event.addListener(marker, 'click', function(event) {
				...
				if(obras[i].img != '' && obras[i].img != null) {
					var img = '<img src="../img/obras/'+obras[i].codigo+'/'+obras[i].img+'" align="right" width="150" alt="Imagem da Obra"  />';
				} else {
					var img = '';
				}
				
				var infowindow = new google.maps.InfoWindow({
					content: '<div style="width: 450px; min-height: 150px;">'+ 
						'<span style="font-family: arial, sans-serif; font-weight: bold;">'+obras[i].nome+'</span><br />'
						+'<span style="font-family: arial, sans-serif; font-size: 10pt">'+obras[i].unidade.compl+'</span><br /><br />'
						+descr
						+img
						+'<b>&Aacute;rea: </b>'+obras[i].area.compl+'<BR />'
						+'<b>Caracter&iacute;stica: </b>'+obras[i].caract.label+'<BR />'
						+'<b>Tipo: </b>'+obras[i].tipo.label+'<BR />'
						+'<b>Estado: </b>'+obras[i].estado.label
						+'</div>',
				});
