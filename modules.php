<?php
	...
	function showMenu($template,$perm,$area,$bd) {
		...
		if ($perm[11]) $data["itens_obra"] .= '<a href="sgo.php?acao=cadastrar">Cadastrar nova obra</a><br />';
		if ($perm[12]) $data["itens_obra"] .= '<a href="sgo.php?acao=buscar">Buscar Obra</a>';
	...
	function montaCampo($c,$tipo = 'cad',$valor = null,$busca = false){
		...
		if ($cp['tipo'] == 'input') {
			...
			if (strpos($cp['extra'],"unOrg_autocompletar") !== false){
				if($tipo == 'edt')
					$campo['cod'] = '<input autocomplete="off" name="'.$cp['nome'].'" id="'.$cp['nome'].'" '.$cp['attr'].' value="'.$campo['valor'].'"/>';
				else
			...
			if($tipo == 'edt') {
				if($valor[$c] == 1)	$campo['cod'] = '<input type="radio" name="'.$cp['nome'].'" id="'.$cp['nome'].'" value="1" ckecked="checked" /> Sim&nbsp;&nbsp;<input type="radio" name="'.$cp['nome'].'" id="'.$cp['nome'].'" value="0" /> N&atilde;o';
				elseif($valor[$c] == 0) $campo['cod'] = '<input type="radio" name="'.$cp['nome'].'" id="'.$cp['nome'].'" value="1" /> Sim&nbsp;&nbsp;<input type="radio" name="'.$cp['nome'].'" id="'.$cp['nome'].'" value="0" ckecked="checked" /> N&atilde;o';
