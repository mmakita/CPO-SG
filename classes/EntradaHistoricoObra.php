...
class EntradaHistoricoObra {
	...
	function __construct($obraID = null, $labelID = null, $dados = null) {
		$this->id          = 0;
		$this->obraID      = $obraID;
	...
	function save() {
		...
		if(!$this->id) {
			if($bd->query("INSERT INTO obra_historico (obraID, labelID, dados, userID, data) VALUES ({$this->obraID}, {$this->label['id']}, '".json_encode($this->dados)."', {$this->user['id']}, ".time().")")) {
